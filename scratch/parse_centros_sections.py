...
def unpack_centros(file_path):
    ...
    if not manifest_match or not template_match:
        print("Erro: script tags do bundler nao encontradas!")
        return
        ...
        if entry.get('compressed'):
            try:
                ...
                try:
                    ...
                    print(f"Decompress error {uuid}: {e}")
    ...
    with open('scratch/extracted_centros_all.html', 'w', encoding='utf-8') as f:
        f.write(final_html)

    headings = re.findall(r'<h[1-6][^>]*>(.*?)</h[1-6]>', final_html, re.DOTALL)
    print("Found Headings in Centros HTML:")
    for h in headings:
...
if __name__ == "__main__":
    unpack_centros(r"C:\Users\Muquissi\Downloads\edukangola-centros.html")
