...
export function NearbyCoursesSection({ data, onAnnounce }) {
  ...
  const locateUser = ({ closePrompt = true } = {}) => {
    if (closePrompt) setLocationPromptOpen(false);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      if (navigator.geolocation) locateUser({ closePrompt: false });
    }, 350);
  ...
  return <>
    ...
    {locationPromptOpen && <div className="location-consent-overlay" role="presentation"><section className="location-consent-dialog" role="dialog" aria-modal="true" aria-labelledby="location-consent-title"><button type="button" className="location-consent-close" onClick={() => setLocationPromptOpen(false)} aria-label="Fechar pedido de localização"><X size={18} /></button><span className="location-consent-icon"><LocateFixed size={23} /></span><span className="location-consent-eyebrow"><ShieldCheck size={14} /> Privacidade primeiro</span><h3 id="location-consent-title">Encontrar centros perto de si?</h3><p>{status === "loading" ? "Abrimos a autorização de GPS. Se o aviso do navegador não aparecer, use o botão abaixo para o pedir novamente." : "Com a sua autorização, usaremos o GPS do navegador apenas nesta pesquisa para procurar cursos e centros próximos. A Edukangola não guarda a sua localização."}</p><div className="location-consent-actions"><button type="button" className="location-consent-secondary" onClick={() => setLocationPromptOpen(false)}>Agora não</button><button type="button" className="location-consent-primary" onClick={() => locateUser({ closePrompt: false })}>{status === "loading" ? <RefreshCw size={16} className="is-spinning" /> : <LocateFixed size={16} />}{status === "loading" ? "Pedir novamente" : "Permitir localização"}</button></div></section></div>}
  </>;
}
