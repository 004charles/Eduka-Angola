...
function CoursesPage({ data, onRefresh }) {
  ...
  return <><PageHeader page="cursos"/><section className="manager-courses manager-module-card"><button className="manager-create-course" onClick={() => setCreating(true)}>Criar curso</button>{error && <p className="manager-form-error">{error}</p>}<div className="manager-course-list manager-catalog-list">{data.cursos.length ? data.cursos.map((course) => <article key={course.id}><div className="manager-course-cover" aria-hidden="true">{course.imagem_url ? <img src={course.imagem_url} alt=""/> : <BookOpen size={20}/>}</div><div className="manager-course-copy"><strong>{course.titulo}</strong><span>{course.categoria} · {money.format(course.preco)} Kz</span><small>{course.ativo ? "Activo no catálogo" : "Inactivo"}</small></div><span className={course.publicado ? "published" : "draft"}>{course.publicado ? "Publicado" : "Rascunho"}</span><div className="manager-course-actions"><button onClick={() => changePublication(course)}>{course.publicado ? "Retirar" : "Publicar"}</button><button onClick={() => setEditingCourseId(course.id)} aria-label={`Editar ${course.titulo}`}><Pencil size={16}/></button><button className="manager-course-delete" onClick={() => deleteCourse(course)} aria-label={`Remover ${course.titulo}`}><Trash2 size={16}/></button></div></article>) : <p className="manager-empty-state">Ainda não existem cursos no centro. Crie a primeira formação para iniciar o catálogo.</p>}</div></section>{creating && <ManagerCourseForm onClose={() => setCreating(false)} onCreated={() => { setCreating(false); onRefresh(); }} />}{editingCourseId && <ManagerCourseForm courseId={editingCourseId} onClose={() => setEditingCourseId(null)} onUpdated={() => { setEditingCourseId(null); onRefresh(); }} />}</>;
}
