...
import HomeDecisionSections from "../components/HomeDecisionSections";
import LearningEntryPoints from "../components/LearningEntryPoints";
...
export default function HomePage({ data, loading, onNavigate, onAnnounce }) {
  ...
  return <main id="top"><CampaignCarousel onNavigate={onNavigate} /><section className="trust-strip"><div className="page-width trust-inner"><span><GraduationCap size={20} /> {t("home.trustCentres")}</span><span><MapPin size={20} /> {t("home.trustVideos")}</span><span><BookOpen size={20} /> {t("home.trustClarity")}</span></div></section><LearningEntryPoints data={data} onNavigate={onNavigate} /><HomeDecisionSections data={data} loading={loading} onAnnounce={onAnnounce} /><section className="page-width category-section home-skills-area"><div className="section-heading"><div><span className="eyebrow muted"><SlidersHorizontal size={14} /> {t("home.eyebrow")}</span><h2>{t("home.title")}</h2><p className="home-category-intro">Comece por uma competência. Depois, compare apenas os cursos e centros que fazem sentido para si.</p></div><button className="text-action" onClick={() => goToCatalogue()}>{t("home.allAreas")} <ArrowRight size={16} /></button></div><div className="category-grid">{categories.map((category) => <button className="category-card" key={category.key} onClick={() => goToCatalogue(category.query)}><span className="category-icon">{category.icon}</span><span><strong>{t(`category.${category.key}`)}</strong><small>{t(`category.${category.key}Copy`)}</small></span><ChevronRight size={17} /></button>)}</div></section><SkillsDiscoverySection data={data} loading={loading} onAnnounce={onAnnounce} /></main>;
}
