...
function CourseTile({ course, onNavigate }) {
  ...
  const inscricao = Number(financeiro.inscricao?.valor) > 0 ? financeiro.inscricao.formatado : "Sem taxa";
  const mensalidade = Number(financeiro.mensalidade?.valor) > 0 ? financeiro.mensalidade.formatado : "";
  return <article className="center-course-tile"><div className="center-course-image">{course.imagem_url ? <img src={course.imagem_url} alt="" /> : <div className="center-course-image-fallback"><BookOpen size={28} /></div>}<span>{course.categoria}</span></div><div className="center-course-copy"><p className="center-course-level">{course.nivel || "Formação profissional"}</p><h3>{course.titulo}</h3>{course.descricao_curta && <p className="center-course-description">{course.descricao_curta}</p>}<div className="center-course-details"><span>{course.carga_horaria && <><Clock3 size={14} /> {course.carga_horaria}</>}</span><strong>Inscrição · {inscricao}{mensalidade && <small>Mensalidade · {mensalidade}</small>}</strong></div><button type="button" onClick={() => onNavigate(`/cursos/${course.slug}`)}>Ver formação <ArrowRight size={15} /></button></div></article>;
}
