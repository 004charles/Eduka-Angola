  ...
  return <main className="center-profile-page">
    ...
    <section className="page-width center-profile-layout">
      <div className="center-profile-main">
        ...
        {center.reels?.length > 0 && <section className="center-profile-section"><SectionHeading eyebrow="Em vídeo" title="Momentos do centro" icon={Play} /><div className="center-profile-reels">{center.reels.map((reel) => <article key={reel.id}>{reel.video_url ? <video controls preload="metadata" poster={reel.thumbnail_url || undefined} src={reel.video_url} /> : reel.thumbnail_url && <img src={reel.thumbnail_url} alt="" />}<div><h3>{reel.titulo}</h3>{reel.descricao && <p>{reel.descricao}</p>}{reel.duracao > 0 && <small>{reel.duracao}s</small>}</div></article>)}</div></section>}
