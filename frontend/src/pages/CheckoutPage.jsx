...
export default function CheckoutPage({ kind, course: initialCourse, turmas = [], slug, student, onNavigate }) {
  ...
  useEffect(() => {
    if (!student || kind === "video") return;
    setForm((current) => ({
      ...current,
      nome: current.nome || student.nome || "",
      email: current.email || student.email || "",
      telefone: current.telefone || student.telefone || "",
    }));
  }, [student, kind]);
  ...
  if (!isVideo && !student) return <main className="detail-state page-width"><span className="eyebrow muted">Inscrição</span><h1>Entre para continuar a inscrição.</h1><p>Precisa de ter uma sessão iniciada para se inscrever nesta formação. Os seus dados serão preenchidos automaticamente.</p><button className="primary-action" onClick={() => onNavigate(`/entrar?next=${encodeURIComponent(window.location.pathname)}`)}>Entrar na conta</button></main>;
