...
categoria, _ = Categoria.objects.get_or_create(slug='telejornal', defaults={'nome': 'Telejornal', 'descricao': 'Notícias e vídeos de demonstração do Edukangola Agora.'})
for slug, titulo, resumo, video_url, duracao, image_name in videos:
    post, created = Post.objects.update_or_create(slug=slug, defaults={
        'titulo': titulo, 'categoria': categoria, 'resumo': resumo, 'conteudo': resumo,
        'imagem_capa': f'static/blog/{image_name}', 'tipo_conteudo': 'video', 'video_url': video_url,
        'duracao_video': duracao, 'status': 'publicado', 'publicado_em': timezone.now(),
    })
    print('created' if created else 'updated', post.slug)
